...
class EMOE(nn.Module):
    ...
    def forward(self, text, audio, video):
        ...
        l_proj = self.proj2_l(
            F.dropout(
                F.relu(
                    self.proj1_l(c_l_att), inplace=True), p=self.output_dropout, training=self.training))
        l_proj += c_l_att
        ...
        a_proj = self.proj2_a(
            F.dropout(
                F.relu(
                    self.proj1_a(c_a_att), inplace=True), p=self.output_dropout, training=self.training))
        a_proj += c_a_att
